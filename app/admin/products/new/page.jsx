  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto px-4 sm:px-0">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="font-serif text-3xl text-primary font-bold">Add New Product</h2>
      ...
      <form 
        onSubmit={(e) => {
          e.preventDefault();
          alert('In a real app, this would upload files and save to a database. Since this is a demo, we are showing the UI flow.');
        }}
      ...
      >
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input type="text" name="name" required placeholder="e.g. Chocolate Fudge Cake" className="w-full rounded-xl border border-border bg-cream px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-hidden transition-all" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-semibold text-primary">Price (₹)</label>
              <input type="number" step="0.01" min="0" name="price" required placeholder="0.00" className="w-full rounded-xl border border-border bg-cream px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-hidden transition-all" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-semibold text-primary">Category</label>
              <select name="category" required className="w-full rounded-xl border border-border bg-cream px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-hidden transition-all appearance-none">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 rounded-xl border border-border bg-cream px-4 py-2.5 text-sm h-[42px]">
                <input type="checkbox" name="featured" id="featured" className="w-4 h-4 rounded text-primary focus:ring-primary/20 accent-primary" />
          ...
          <div className="space-y-2">
            ...
            <textarea name="description" rows={4} placeholder="Describe the product, ingredients, etc." className="w-full rounded-xl border border-border bg-cream px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 outline-hidden transition-all resize-none" />
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <p className="text-xs text-muted-foreground -mt-4">Select high-quality images from your directory</p>
              ...
              <div key={key} className="space-y-3">
                ...
                <div className="relative aspect-square rounded-2xl border-2 border-dashed border-border bg-muted/30 overflow-hidden group hover:border-primary/50 transition-colors">
                  {previews[key] ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                      <span className="text-xs text-muted-foreground text-center">Optional gallery image</span>
                    </div>
                  )}
                  <input 
                    type="file" 
